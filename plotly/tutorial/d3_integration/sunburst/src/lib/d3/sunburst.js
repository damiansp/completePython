...
export default class Sunburst3D {
    ...
    _update(fig) {
        ...
        const textTrans = node => {
            const rot = (
                (angleCenter(node) * 360 / Tau + (skinny(node) ? 0 : 90))
                % 180 - 90);
        ...
        function wrap(accessor) {
            return d => {
                return t => {
                    ...
                    if (d0 && d0 !== d) {
                        const interpolator = d3.interpolateObject(posOnly(d0),
                                                                  posOnly(d));
        ...
        const transitionToNode = node => {
            ...
            // mostly self._promise takes care of this; want to avoid clicks
            // during transitions
            ...
            const transition = self.svg.transition()
                  ...
                  .tween('scale', () = {
                      const angularDomain = d3.interpolate(
                          self.angularScale.domain(), selectedX(node));
                      const radialDomain = d3.interpolate(
                          self.radialScale.domain(), selectedY(node));
                      const radialRange = d3.interpolate(
                          self.radialScale.range(), selectedRadius(node));
                      return function(t) {
                          self.angularScale.domain(angularDomain(t));
                          self.radialScale.domain(
                              radialDomain(t)).range(radialRange(t));
                      };
                  });
        ...
        const updatePaths = (_paths, _texts, _dataChange) => {
            if (_dataChange) {
                ...
                _texts.enter().append('text').style(textStyle).text(
                    d => d.name);
            }

            // update to attributes; need regardless of what changed
            _paths.attr('d', self.arc)
            // coloring this way is history-dependent. if new item is inserted
            // in the middle it will get the next color, existing items keep
            // their colors.
            // But later redrawing of this component straight from data will
            ...
            // `d3.on('mouseover')` and draw tooltip. requires extra logic to
            // work correctly across state updates
            _paths.select('title')
                .text(d => d.name + '\n' + numFormat(d.value));
        ...
        // no node: path is wrong, prob received new selectedPath before its
        // data
        ...
        const shouldAnimate = (
            self.initialized // first draw
                ...
                // not pure up/down trans
                && sameHead(oldSelectedPath, newSelectedPath) 
                // the prev data didn't contain the new selected node; can
                // happen if we transition selectedPath first, then data
        ...
        if (shouldAnimate) {
            ...
            // first draw has no animation and inits scales
            self.angularScale.domain(selectedX(selectedNode));
            self.radialScale.doman(selectedY(selectedNode));
