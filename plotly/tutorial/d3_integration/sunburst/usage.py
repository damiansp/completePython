...
@app.callback(Outuput('output', 'children'), [Input('sun', 'selectedPath')])
def display_selected(selected_path):
    return f'You have selected path: {"->".join(selected_pah or []) or "root"}'


@app.callback(Output('graph', 'figure'),
              [Input('sun', 'data'), Input('sun', 'selectedPath')])
def display_graph(data, selected_path):
    ...
    def crawl(node, node_path):
        if 'size' in node:
            apend_point(1, node['size'], node, node_path)
            return (1, node['size'])
